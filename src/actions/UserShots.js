...
const loadShots = (dispatch, userId, page) => {
  api.listUserShots(userId, page)
    .then(data => {
...
const fetchFromBottom = (userId, page) => {
  return (dispatch) => {
    dispatch({type: ShotsTypes.SCROLL_TO_BOTTOM});
    loadShots(dispatch, userId, page + 1);
  };
};

const refreshShots = (userId) => {
  return (dispatch) => {
    dispatch({type: ShotsTypes.PULL_TO_REFRESH});
    loadShots(dispatch, userId, 0);
  }
};
